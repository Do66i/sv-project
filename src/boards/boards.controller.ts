...
export class BoardsController {
    ...
    // 4. 게시글 생성 (POST 요청 처리)
    @Post() // 'localhost:3000/boards' 경로로 POST 요청이 들어오면 이 함수가 실행됨
    createBoard(
        @Body('title')title: string, // 사용자가 보낸 Body에서 'title'만 뽑아옴
        @Body('description')description: string // 사용자가 보낸 Body에서 'description'만 뽑아옴
    ): Board {
        this.logger.log('----- 게시글 생성 요청 (POST) -----');
        this.logger.log(
            `입력 데이터: title=${title}, description=${description}`,
        );
        const result = this.boardsService.createBoard(title, description);
        this.logger.log(`생성 결과: ${JSON.stringify(result)}`);
        this.logger.log('-----------------------------------');
        // 일꾼에게 '이 데이터로 게시글 하나 만들어줘'라고 시킨다
